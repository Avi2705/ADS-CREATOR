...
const seedDatabase = async () => {
  try {
    ...
    const b2cTenant = await Tenant.create({
      name: 'Velocity Sportswear & Footwear',
      type: 'B2C'
    });

    const b2bTenant = await Tenant.create({
    ...
    // Staff Employee
    const staffEmployee = await User.create({
      name: 'Sarah Jenkins',
    ...
    // B2C Customer
    const b2cCustomer = await User.create({
      name: 'David Miller',
    ...
    // B2B Customer
    const b2bCustomer = await User.create({
      name: 'Marcus Vance',

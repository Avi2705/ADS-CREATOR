...
exports.ensureMetaCompatibleImageUrl = ensureMetaCompatibleImageUrl;
...
const adBannerRenderer_service_1 = require("../creatives/adBannerRenderer.service");
...
// HELPERS
// ============================================================
function createMetaError(data, fallback) {
...
function validateToken(token, platform) {
    if (!token) {
        throw new Error(`${platform} access token is missing`);
    }
    if (typeof token !== 'string' ||
        token.trim().length < 20) {
        throw new Error(`${platform} access token is invalid or incomplete`);
    }
}
function validateMediaUrl(mediaUrl) {
    ...
    if (mediaUrl.startsWith('data:')) {
        throw new Error('Base64 media is not supported directly. Upload the image to Cloudinary, S3, or another public storage service first.');
    }
    let url;
    try {
    ...
    if (hostname === 'localhost' ||
        ...
        throw new Error('Meta cannot access localhost. Use a public Cloudinary/S3/CDN URL.');
    }
    return mediaUrl;
}
// ============================================================
...
// ============================================================
async function facebookGetPage(pageId, accessToken) {
    validateToken(accessToken, 'Facebook');
    if (!pageId) {
        throw new Error('Facebook Page ID is required');
    }
    const url = `${FACEBOOK_GRAPH_URL}/${encodeURIComponent(pageId)}` +
        `?fields=id,name&access_token=${encodeURIComponent(accessToken)}`;
    console.log(`[Facebook Page Verification] Checking Page ID: ${pageId}`);
    const response = await fetchFn(url);
    const data = (await response.json());
    console.log(`[Facebook Page Verification] HTTP ${response.status}`, data);
    if (!response.ok ||
        data.error) {
        throw createMetaError(data, 'Facebook Page verification failed');
    }
    if (!data.id) {
        throw new Error('Facebook Page ID was not returned');
    }
    return {
        id: String(data.id),
...
// ============================================================
async function instagramGetAccount(accessToken) {
    validateToken(accessToken, 'Instagram');
    const isMetaToken = accessToken.startsWith('EAA');
    const primaryUrl = isMetaToken
        ? FACEBOOK_GRAPH_URL
        : INSTAGRAM_GRAPH_URL;
    console.log(`[Instagram OAuth Debug] Using API: ${primaryUrl} (token prefix: ${accessToken.slice(0, 10)}...)`);
    const url = `${primaryUrl}/me` +
    ...
    if (!response.ok ||
        data.error) {
        console.error(`[Instagram OAuth Failure]`, data.error);
        throw createMetaError(data, 'Instagram token verification failed');
    }
    const userId = data.user_id || data.id;
    const username = data.username || data.name;
    if (!userId) {
        throw new Error('Instagram user_id or username was not returned');
    }
    return {
        user_id: String(userId),
        username: String(username || 'instagram_user'),
    };
}
function ensureMetaCompatibleImageUrl(url) {
    if (!url)
        return url;
...
// ============================================================
async function instagramCreateContainer(igUserId, accessToken, imageUrl, caption, mediaType) {
    validateToken(accessToken, 'Instagram');
    if (!igUserId) {
        throw new Error('Instagram User ID is required');
    }
    const compatibleImageUrl = ensureMetaCompatibleImageUrl(imageUrl);
    validateMediaUrl(compatibleImageUrl);
    ...
    const isMetaToken = accessToken.startsWith('EAA');
    const baseUrl = isMetaToken
        ? FACEBOOK_GRAPH_URL
        : INSTAGRAM_GRAPH_URL;
    const response = await fetchFn(`${baseUrl}/${encodeURIComponent(igUserId)}/media`, {
        method: 'POST',
        headers: {
    ...
    if (!response.ok ||
        data.error) {
        console.error(`[Instagram Container Creation Failure]`, data.error);
        throw createMetaError(data, 'Instagram media container creation failed');
    }
    if (!data.id) {
        throw new Error('Instagram did not return a container ID');
    }
    console.log(`[Meta Media Container Created] Container ID: ${data.id}`);
    return data.id;
}
// ============================================================
// INSTAGRAM - WAIT FOR CONTAINER STATUS
// ============================================================
async function instagramWaitForContainer(containerId, accessToken) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        const url = `${baseUrl}/${encodeURIComponent(containerId)}` +
        ...
        if (!response.ok ||
            data.error) {
            console.error(`[Instagram Container Status Check Failure]`, data.error);
            throw createMetaError(data, 'Failed to check Instagram media container status');
        }
        console.log(`[Instagram Container Status] Attempt ${attempt}: status_code=${data.status_code || 'N/A'}, status=${data.status || 'N/A'}`);
        if (data.status_code ===
            'FINISHED') {
            return;
        }
        if (data.status_code === 'ERROR' ||
            ...
            throw new Error(`Instagram media container failed with status: ${data.status_code}`);
        }
...
async function instagramPublishContainer(igUserId, accessToken, containerId) {
    ...
    const baseUrl = isMetaToken
        ? FACEBOOK_GRAPH_URL
        : INSTAGRAM_GRAPH_URL;
    const response = await fetchFn(`${baseUrl}/${encodeURIComponent(igUserId)}/media_publish`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
    ...
    if (!response.ok ||
        data.error) {
        console.error(`[Instagram Publish Failure]`, data.error);
        throw createMetaError(data, 'Instagram publishing failed');
    }
    if (!data.id) {
        throw new Error('Instagram did not return a published post ID');
    }
    return data.id;
}
// ============================================================
...
// ============================================================
async function facebookPublishPhoto(pageId, accessToken, imageUrl, caption) {
    validateToken(accessToken, 'Facebook');
    if (!pageId) {
        throw new Error('Facebook Page ID is required');
    }
    ...
    const response = await fetchFn(`${FACEBOOK_GRAPH_URL}/${encodeURIComponent(pageId)}/photos`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
    ...
    if (data.id && !data.post_id) {
        try {
            ...
            const feedRes = await fetchFn(`${FACEBOOK_GRAPH_URL}/${encodeURIComponent(pageId)}/feed`, {
                method: 'POST',
    ...
    try {
        ...
        if (!userId ||
            ...
            return res.status(400).json({
                success: false,
                error: 'userId, platform, handle, accountId and accessToken are required',
            });
        }
        ...
        // UPSERT ACCOUNT
        // ========================================================
        const account = await social_model_1.default.findOneAndUpdate({ userId, platform }, {
            handle,
            accountId,
            accessToken,
...
async function disconnectAccount(req, res) {
    try {
        ...
        await social_model_1.default.findOneAndUpdate({
            userId: userId,
            platform: platform,
        }, {
            isConnected: false,
...
async function getConnectedAccounts(req, res) {
    try {
        ...
        const accounts = await social_model_1.default.find({
            userId,
            isConnected: true,
        }).select('-accessToken');
        return res.status(200).json({
...
async function publishPost(req, res) {
    try {
        const { userId, channels, headline = '', caption = '', mediaUrl, targetUrl = '', adId, mediaType = 'IMAGE', status = 'PUBLISHED', scheduledDate, } = req.body;
        if (!userId ||
            ...
            return res.status(400).json({
                success: false,
                error: 'userId, channels and mediaUrl are required',
            });
        }
        // ========================================================
        // MEDIA
        // ========================================================
        let resolvedMediaUrl = validateMediaUrl(mediaUrl);
        // ========================================================
        ...
        if (mediaType !== 'VIDEO' &&
            ...
            try {
                const compositeBanner = await (0, adBannerRenderer_service_1.renderAndUploadAdBanner)({
                    headline: headline ||
                ...
                if (compositeBanner &&
                    compositeBanner.startsWith('http')) {
                    resolvedMediaUrl =
                        compositeBanner;
                    console.log(`[Publish Post Renderer] Created Composite Banner: ${resolvedMediaUrl}`);
                }
            }
            catch (renderErr) {
                console.warn('[Publish Post Renderer] Banner overlay notice:', renderErr);
            }
        }
        // ========================================================
        ...
        for (const channel of uniqueChannels) {
            const account = connectedAccounts.find((item) => item.platform ===
                channel);
            // ======================================================
            ...
            if (!account) {
                results[channel] = {
                    success: false,
                    postId: null,
                    error: `${channel} account is not connected`,
                };
                continue;
            }
            try {
                ...
                if (channel ===
                    'Instagram') {
                    const instagramAccount = await instagramGetAccount(account.accessToken);
                    const targetIgUserId = account.accountId ||
                    ...
                    const fullCaption = [
                        ...
                        .join('\n\n');
                    // STEP 1
                    const containerId = await instagramCreateContainer(targetIgUserId, account.accessToken, resolvedMediaUrl, fullCaption, mediaType);
                    console.log(`[Instagram] Container created: ${containerId}`);
                    // STEP 2
                    await instagramWaitForContainer(containerId, account.accessToken);
                    console.log(`[Instagram] Container ready: ${containerId}`);
                    // STEP 3
                    const postId = await instagramPublishContainer(targetIgUserId, account.accessToken, containerId);
                    console.log(`[Instagram] Published successfully: ${postId}`);
                    results[channel] = {
                        success: true,
                        postId,
                        containerId,
                ...
                // ====================================================
                    ...
                    if (!account.accountId) {
                        throw new Error('Facebook Page ID is missing from connected account');
                    }
                    if (!account.accessToken) {
                        throw new Error('Facebook Page Access Token is missing from connected account');
                    }
                    // --------------------------------------------------
                    // Verify Page before publishing
                    // --------------------------------------------------
                    const page = await facebookGetPage(account.accountId, account.accessToken);
                    console.log(`[Facebook] Verified Page: ${page.name} (${page.id})`);
                    const fullCaption = [
                        headline,
                        caption,
                        targetUrl,
                    ...
                    // Publish photo
                    // --------------------------------------------------
                    const postId = await facebookPublishPhoto(page.id, account.accessToken, resolvedMediaUrl, fullCaption);
                    results[channel] = {
                        success: true,
                        postId,
                    };
                ...
                // ====================================================
                    ...
                    const waAccount = await social_model_1.default.findOne({ userId, platform: 'WhatsApp', isConnected: true });
                ...
                // ====================================================
                    ...
                    results[channel] = {
                        success: false,
                        postId: null,
                        error: `${channel} publishing is not implemented`,
                    };
                }
            }
            catch (channelErr) {
                console.error(`========== ${channel.toUpperCase()} API ERROR ==========`);
                console.error('Message:', channelErr.message);
                console.error('Meta:', channelErr.meta);
                console.error('Full error:', channelErr);
                console.error('================================================');
                results[channel] = {
                    success: false,
                    postId: null,
        ...
        const failedCount = Object.values(results).filter(result => result.success === false).length;
        // ========================================================
        // STATUS
        // ========================================================
        ...
        // RESPONSE
        // ========================================================
        return res.status(successCount > 0
            ? 200
            : 502).json({
...
async function getSocialPosts(req, res) {
    try {
        ...
        let posts = await social_model_1.SocialPost.find(query).sort({
            createdAt: -1,
        });
        // Fallback
        if (posts.length === 0) {
            posts =
                await social_model_1.SocialPost.find({}).sort({
                    createdAt: -1,
                });
        }
        console.log(`[Social Posts Fetch] Query:`, query, `Found ${posts.length} posts in MongoDB`);
        return res.status(200).json({
            success: true,
            posts,
        });
...
// ============================================================
async function sendWhatsAppCloudMessage(phoneNumberId, accessToken, recipientNumber, messageText, mediaUrl, templateName) {
    try {
        ...
        const sendReq = async (bodyPayload) => {
            const res = await fetchFn(`${FACEBOOK_GRAPH_URL}/${encodeURIComponent(phoneNumberId)}/messages`, {
                method: 'POST',
                headers: {
 ...
 */
async function sendWhatsAppBroadcast(req, res) {
    try {
        ...
        if (!activePhoneId || !activeToken) {
            const account = await social_model_1.default.findOne({ userId, platform: 'WhatsApp', isConnected: true });
            if (account) {
        ...
        const numbersList = Array.isArray(toNumbers) ? toNumbers : [toNumbers];
        const results = [];

...
exports.processWhatsAppWebhookStatus = processWhatsAppWebhookStatus;
...
const adInteraction_model_1 = __importDefault(require("../leads/adInteraction.model"));
const mongoose_1 = __importDefault(require("mongoose"));
const mail_service_1 = require("../../services/mail.service");
const metaInsights_service_1 = require("../social/metaInsights.service");
const router = (0, express_1.Router)();
/**
 * Helper: Process WhatsApp Webhook Delivery Status Events (sent, delivered, read, failed)
 */
async function processWhatsAppWebhookStatus(body) {
    ...
    for (const entry of entries) {
        ...
        for (const change of changes) {
            ...
            for (const statusObj of statuses) {
                ...
                const post = await social_model_1.SocialPost.findOne({
                    $or: [
                ...
                if (post) {
                    const delivery = post.whatsappDelivery || { messageId, recipient };
 ...
 */
router.get(['/whatsapp', '/facebook'], (req, res) => {
    try {
 ...
 */
router.post(['/whatsapp', '/facebook'], async (req, res) => {
    try {
        const body = req.body;
                ...
                if (entry.changes) {
                    for (const change of entry.changes) {
                        ...
                        if (field === 'leadgen') {
                            ...
                            const metaLead = await (0, metaInsights_service_1.fetchMetaLeadDetails)(leadgenId, token);
                            if (!metaLead) {
                                ...
                                await adInteraction_model_1.default.create({
...
router.post('/simulate', async (req, res) => {
    try {
        ...
        let query = {};
        if (postId) {
            query = { $or: [{ _id: mongoose_1.default.Types.ObjectId.isValid(postId) ? postId : undefined }, { postId }, { platformPostId: postId }] };
        }
        else if (adId) {
            query = { adId };
        }
        ...
        const updated = await social_model_1.SocialPost.findOneAndUpdate(query, { $inc: { [incField]: Number(count) } }, { new: true, sort: { createdAt: -1 } });
...
//# sourceMappingURL=fbWebhook.routes.js.map